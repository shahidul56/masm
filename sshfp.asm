...
data segment
	...
	err_general_info		db 'Version number (0 or 1) and 32 hexadecimal digits [0-9a-f] required.$' 
	err_wrong_version		db 'Version number must be either 0 or 1.$'
	err_incorrect_hex		db 'Incorrect SSH fingerprint. Use only hexadecimal digits [0-9a-f].$'
...
;------------------------------------------------------------------------------
; EXIT_ON_INPUT_ERROR source
; Same as above but adds general info about requirements of arguments.
;------------------------------------------------------------------------------
exit_on_input_error macro source
	mov		si, offset source
	call	print_str
...
read_args proc	
	...
	; If it is CR it means that user provided at least one argument but there
	; are no more.
	cmp		al, CR
	je		check_last_arg
	...
	; Or maybe the number of expected arguments is already in memory, which
	; means invalid input.
	push	ax
	...
	catch_err_too_long:
	exit_on_input_error err_too_long
	
	catch_err_too_many_args:
	exit_on_input_error err_too_many_args
read_args endp
...
parse_args proc
	...
	exit_on_input_error err_not_enough_args
	
	catch_error_wrong_args_lengths:
	exit_on_input_error err_wrong_args_lengths
	
	catch_err_wrong_version:
	exit_on_error err_wrong_version
	
	catch_err_incorrect_hex:
	exit_on_error err_incorrect_hex
parse_args endp
...
hex_pair_to_byte proc
	; Check if given hex digit is decimal digit or letter and substract
	; accordingly.
	;
